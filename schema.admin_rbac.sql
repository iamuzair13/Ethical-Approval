...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'admin_role') THEN
    CREATE TYPE admin_role AS ENUM ('administrator', 'hod', 'ireb');
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'admin_assignment_type') THEN
    CREATE TYPE admin_assignment_type AS ENUM ('hod_primary', 'ireb_scope');
  END IF;
END
$$;
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_hod_faculty_single_active
ON admin_faculty_assignments(faculty_id)
WHERE assignment_type = 'hod_primary' AND deleted_at IS NULL;
