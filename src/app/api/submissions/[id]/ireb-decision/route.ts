...
export async function POST(
  ...
  if (!canAccessSubmissionStage(admin, "ireb")) {
  ...
  if (submission.current_status !== "under_ireb_review") {
    return NextResponse.json(
      { ok: false, error: "Submission is not in IREB review stage." },
      { status: 409 },
    );
  }
