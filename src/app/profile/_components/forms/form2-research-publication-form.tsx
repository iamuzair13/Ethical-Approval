...
export function Form2ResearchPublicationForm({
  ...
  if (currentStep === 0) {
    return (
      <section className="grid gap-6">
        <h3 className="text-xl font-bold text-dark dark:text-white">Step 1: Scholar&apos;s Information</h3>
  ...
  if (currentStep === 1) {
    return (
      <section className="grid gap-4 md:grid-cols-2">
        ...
        <select value={form.publicationHumanSubjects} onChange={onFieldChange("publicationHumanSubjects")} className="rounded-lg border border-stroke bg-transparent px-4 py-2.5 dark:border-dark-3"><option value="">2.1 Human participants involved?</option><option>Yes</option><option>No</option></select>
        ...
        <select value={form.preApprovalDataCollected} onChange={onFieldChange("preApprovalDataCollected")} className="rounded-lg border border-stroke bg-transparent px-4 py-2.5 dark:border-dark-3"><option value="">2.4 Data collected before approval?</option><option>Yes</option><option>No</option></select>
        <select value={form.canWithdraw} onChange={onFieldChange("canWithdraw")} className="rounded-lg border border-stroke bg-transparent px-4 py-2.5 dark:border-dark-3"><option value="">2.5 Participants can withdraw?</option><option>Yes</option><option>No</option></select>
        ...
        <select value={form.vulnerablePopulation} onChange={onFieldChange("vulnerablePopulation")} className="rounded-lg border border-stroke bg-transparent px-4 py-2.5 dark:border-dark-3"><option value="">2.8 Vulnerable populations?</option><option>Yes</option><option>No</option><option>N/A</option></select>
      </section>
    );
  }
  ...
  if (currentStep === 2) {
    return (
      <section className="grid gap-4 md:grid-cols-2">
        <h3 className="text-xl font-bold text-dark dark:text-white md:col-span-2">Step 3: Institutional Approvals & Collaboration</h3>
        ...
        <select value={form.internationalCollaboration} onChange={onFieldChange("internationalCollaboration")} className="rounded-lg border border-stroke bg-transparent px-4 py-2.5 dark:border-dark-3"><option value="">International collaboration?</option><option>Yes</option><option>No</option></select>
  ...
  return (
    <section className="grid gap-4">
      ...
      <textarea value={form.declaration} onChange={onFieldChange("declaration")} rows={6} placeholder="I/We certify this manuscript is original and all ethical requirements are met." className="rounded-lg border border-stroke bg-transparent px-4 py-2.5 dark:border-dark-3" />
      <label className="flex items-center gap-2 rounded border border-stroke px-3 py-2 dark:border-dark-3"><input type="checkbox" checked={form.publicationDeclarationAccepted === "yes"} onChange={(e) => setForm((prev) => ({ ...prev, publicationDeclarationAccepted: e.target.checked ? "yes" : "" }))} /><span className="text-sm">Submit</span></label>
    </section>
  );
}
