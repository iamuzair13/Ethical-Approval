...
export function Form3ThesisMedicalForm({
    ...
    return (
      <section className="grid gap-6">
        <h3 className="text-xl font-bold text-dark dark:text-white">
          Step 1: Researcher(s) and Thesis/Project Information <RequiredMark />
        </h3>
        ...
        <div className="grid gap-4 rounded-lg border border-stroke p-4 dark:border-dark-3 md:grid-cols-2">
          <h4 className="font-semibold text-dark dark:text-white md:col-span-2">1.2 Supervisor(s)&apos;s Information <RequiredMark /></h4>
  ...
  if (currentStep === 1) {
    return (
      <section className="grid gap-4 md:grid-cols-2">
        ...
        <select required value={form.involveHumanParticipants} onChange={onFieldChange("involveHumanParticipants")} className="rounded-lg border border-stroke bg-transparent px-4 py-2.5 dark:border-dark-3"><option value="">2.1 Human subjects/patients?</option><option>Yes</option><option>No</option></select>
        <select required value={form.collectPii} onChange={onFieldChange("collectPii")} className="rounded-lg border border-stroke bg-transparent px-4 py-2.5 dark:border-dark-3"><option value="">2.2 Collect PII?</option><option>Yes</option><option>No</option></select>
  ...
  if (currentStep === 2) {
    return (
      <section className="grid gap-4 md:grid-cols-2">
        <h3 className="text-xl font-bold text-dark dark:text-white md:col-span-2">Step 3: Biomedical & Pharmaceutical Aspects <RequiredMark /></h3>
        <select required value={form.publicationPharmaInterventions} onChange={onFieldChange("publicationPharmaInterventions")} className="rounded-lg border border-stroke bg-transparent px-4 py-2.5 dark:border-dark-3"><option value="">Involve pharmaceutical substances/drug administration?</option><option>Yes</option><option>No</option></select>
        <input required value={form.drugName} onChange={onFieldChange("drugName")} placeholder="Drug name/classification *" className="rounded-lg border border-stroke bg-transparent px-4 py-2.5 dark:border-dark-3" />
      </section>
    );
  }
  ...
  if (currentStep === 4) {
    return (
      <section className="grid gap-4 md:grid-cols-2">
        <h3 className="text-xl font-bold text-dark dark:text-white md:col-span-2">Step 5: Institutional Approvals & Collaborations <RequiredMark /></h3>
  ...
  if (currentStep === 5) {
    return (
      <section className="grid gap-4">
        <h3 className="text-xl font-bold text-dark dark:text-white">Step 6: Required Attachments <RequiredMark /></h3>
  ...
  return (
    <section className="grid gap-4">
      <h3 className="text-xl font-bold text-dark dark:text-white">Step 7: Declaration and Submission <RequiredMark /></h3>
      <textarea required value={form.declaration} onChange={onFieldChange("declaration")} rows={6} placeholder="Declaration for medical thesis form *" className="rounded-lg border border-stroke bg-transparent px-4 py-2.5 dark:border-dark-3" />
    </section>
  );
}
