...
const STATUS_COLORS: Record<string, string> = {
  "Pending HOD Review": "#fca708",
  "Pending IREB Review": "#f7c060",
  Pending: "#fca708",
  "Approved by HOD": "#2aa33f",
  "Approved by IREB": "#39f029",
  Approved: "#2aa33f",
  "Rejected by HOD": "#ff0703",
...
export function colorForUsedDeviceStatus(name: string): string {
  ...
  if (lower.includes("reject")) {
    return lower.includes("ireb") ? STATUS_COLORS["Rejected by IREB"] : STATUS_COLORS["Rejected by HOD"];
  }
  if (lower.includes("approv")) {
    return lower.includes("ireb") ? STATUS_COLORS["Approved by IREB"] : STATUS_COLORS["Approved by HOD"];
  }
  if (lower.includes("pending")) {
    return lower.includes("ireb") ? STATUS_COLORS["Pending IREB Review"] : STATUS_COLORS["Pending HOD Review"];
  }
