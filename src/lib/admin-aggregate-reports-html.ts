...
function collectHodDecisionDays(rows: AggregateSubmissionInput[]): number[] {
  const out: number[] = [];
  for (const r of rows) {
...
/** Submission → final recorded decision (IREB preferred; else hod), including hod-only rejections. */
...
function hodIrebDelaySharePct(rows: AggregateSubmissionInput[]): { hod: number; ireb: number } | null {
  let sumHod = 0;
  let sumIreb = 0;
  for (const r of rows) {
    if (!r.hod_decision_at || !r.ireb_decision_at) continue;
    sumHod += daysBetween(r.submitted_at, r.hod_decision_at);
    sumIreb += daysBetween(r.hod_decision_at, r.ireb_decision_at);
  }
  const total = sumHod + sumIreb;
  if (total <= 0) return null;
  return { hod: (sumHod / total) * 100, ireb: (sumIreb / total) * 100 };
}
...
function efficiencyMetricsTable(rows: AggregateSubmissionInput[]): string {
  ...
  const rejected =
    rows.filter((r) => r.current_status === "rejected" || r.current_status === "hod_rejected")
      .length;
  ...
  const dd = collectHodDecisionDays(rows);
  ...
  const share = hodIrebDelaySharePct(rows);
  const avgProcDays = mean(terminalCycle);
  const avgHodDays = mean(dd);
  ...
  const rowsHtml: [string, string][] = [
    ...
    ["Avg. delay caused by HOD", avgHodDays != null ? `${fmtDays(avgHodDays)} days` : "—"],
    ["Avg. delay caused by IREB", avgIrebPhaseDays != null ? `${fmtDays(avgIrebPhaseDays)} days` : "—"],
    ["Delay by Faculty (HOD)", share ? fmtPct(share.hod) : "—"],
...
function facultyAggregates(rows: AggregateSubmissionInput[]): Map<string, FacultyInsightAgg> {
  ...
  for (const r of rows) {
    ...
    if (r.current_status === "rejected" || r.current_status === "hod_rejected") cur.rejected += 1;
    m.set(fac, cur);
  }
  return m;
}
