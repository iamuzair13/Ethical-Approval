...
export function buildHodReportChartsHtml(metrics: {
  ...
  if (rows.length === 0 && institutionTotalSubmissions === 0) {
    return reportChartsShell(figureWrap(`${figCaption("HOD scope")}${emptyChartsMessage()}`), 0);
  }
  ...
  const fig1 =
    rows.length > 0
      ? stackedShareBar("Approved vs Rejected (scoped submissions)", [
          { label: "Approved", value: passedHod, color: CHART_COLORS[0]! },
          { label: "Rejected", value: hodRejected, color: CHART_COLORS[4]! },
        ])
  ...
  const fig2 =
    inst > 0
      ? stackedShareBar("Institution submissions: this hod’s scope vs other", [
          { label: "This hod’s scope", value: scoped, color: CHART_COLORS[0]! },
...
export function buildTotalEfficiencyChartsHtml(rows: AggregateSubmissionInput[]): string {
  ...
  const rejected = rows.filter((r) => r.current_status === "rejected" || r.current_status === "hod_rejected").length;
  ...
  const fig2 = stackedShareBar("Outcome-style split (approved vs rejected vs still pending)", [
    ...
    { label: "Rejected (IREB or hod)", value: rejected, color: CHART_COLORS[4]! },
...
function facultyDeptOutcomeCharts(rows: AggregateSubmissionInput[]): string {
  ...
  const pending = rows.filter(
    (r) => r.current_status !== "approved" && r.current_status !== "rejected" && r.current_status !== "hod_rejected",
  ).length;
