import type { AdminRejectionInput, MailPayload } from "../types";
import { escapeHtml, plainTextToHtmlParagraphs } from "../html-escape";

export function buildAdminRejectionEmail(input: AdminRejectionInput): MailPayload {
  const subject = "IERB Ethical Approval Application Returned (Administrator)";
...
<p>We are writing to inform you that your Ethical Approval application has been returned by the Administrator (IREB Chairman). The reason(s) for returning the application are as follow:</p>
${reasonsHtml}
<p>${loginLine}</p>
...
The University of Lahore</p>
  ...
  const text = [
    ...
    "",
    "We are writing to inform you that your Ethical Approval application has been returned by the Administrator (IREB Chairman). The reason(s) for returning the application are as follow:",
    "",
