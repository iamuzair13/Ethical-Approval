import type { HodRejectionInput, MailPayload } from "../types";
import { escapeHtml, plainTextToHtmlParagraphs } from "../html-escape";

export function buildHodRejectionEmail(input: HodRejectionInput): MailPayload {
  const subject = "IERB Ethical Approval Application Rejected (HOD)";
...
<p>We are writing to inform you that your Ethical Approval application has been returned. There were some discrepancies found in the data, due to which ${escapeHtml(input.facultyName)} HOD ${escapeHtml(input.hodName)} has rejected your application. The reason(s) for application rejection is as follow:</p>
${reasonsHtml}
<p>${loginLine}</p>
...
The University of Lahore</p>
  ...
  const text = [
    ...
    "",
    `We are writing to inform you that your Ethical Approval application has been returned. There were some discrepancies found in the data, due to which ${input.facultyName} HOD ${input.hodName} has rejected your application. The reason(s) for application rejection is as follow:`,
    "",
